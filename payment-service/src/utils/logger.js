...
// ---------------------------------------------------------------------------
// RabbitMQ transport — fire-and-forget publish to pharmaflow.logs exchange
// Safe to use before the connection is ready (publishes are no-ops until then)
// ---------------------------------------------------------------------------
...
(async () => {
    try {
        ...
        // Surface channel-level errors so we know if publish starts failing
        ch.on('error', (err) => {
    ...
    log(info, callback) {
        if (channel) {
            try {
                ...
                console.log(`[logger:${SERVICE}] Attempting to publish to RabbitMQ...`);
                ...
                if (!ok) {
                    ...
                    console.log(`[logger:${SERVICE}] Published successfully!`);
                }
            } catch (err) {
